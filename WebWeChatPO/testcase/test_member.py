...
class TestMember:
    ...
    def test_edit_member_name(self, oldname, newname):
        ...
        if oldname in name_list:
            if newname not in name_list:
                ...
                name_list = self.main_page.goto_contact().goto_member_info(
                    oldname).goto_edit_memberinfo().edit_memberinfo_save(newname).goback_contact().get_contact_list()
            else:
                print("新姓名已存在")
                return False
        else:
    ...
    def test_contact_delete_member(self, username):
        ...
        if username in name_list:
            ...
            name_list = self.main_page.goto_contact().goto_member_info(username).delete_member(
                username).get_contact_list()
        else:
            # 如果人员不存在，则返回提示
            print("人员不存在")
            return False
    ...
    def test_contact_delete_member(self, username):
        ...
        if username in name_list:
            ...
            self.main_page.goto_contact().select_delete_member().get_contact_list()
        else:
            # 如果人员不存在，则返回提示
            print("人员不存在")
            return False
